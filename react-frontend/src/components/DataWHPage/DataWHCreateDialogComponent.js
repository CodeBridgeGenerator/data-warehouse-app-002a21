...
const DataWHCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceName: _entity?.serviceName,count: _entity?.count,data: _entity?.data,timestamps: _entity?.timestamps || false,
...
<div className="col-12 md:col-6 field flex mt-5">
            <span className="align-items-center">
                ...
                <Checkbox id="timestamps" className="ml-3" checked={_entity?.timestamps} onChange={(e) => setValByKey("timestamps", e.checked)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["timestamps"]) ? (
              <p className="m-0" key="error-timestamps">
                {error["timestamps"]}
              </p>
            ) : null}
          </small>
